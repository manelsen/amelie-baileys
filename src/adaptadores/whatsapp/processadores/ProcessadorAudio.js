  ...
  const processarMensagemAudio = async (dados) => {
    ...
    try {
      ...
      if (!config || !config.mediaAudio) {
        registrador.debug(`[Aud] Transcrição DESABILITADA. Ignorando.`); // Simplificado
      ...
      if (!resultadoRemetente.sucesso) {
           registrador.error(`[Aud] Falha ao obter remetente: ${resultadoRemetente.erro?.message}`);
      ...
      if (!resultadoTamanho.sucesso) {
        registrador.warn(`[Aud] Áudio muito grande: ${resultadoTamanho.erro.message}`); // Simplificado
      ...
      const resultadoTransacao = await gerenciadorTransacoes.criarTransacao(mensagem, chat);
       if (!resultadoTransacao.sucesso) {
           registrador.error(`[Aud] Falha ao criar transação: ${resultadoTransacao.erro?.message}`);
      ...
      if (!resultadoIA.sucesso) {
        registrador.error(`[Aud] Falha no adaptadorIA.processarAudio: ${resultadoIA.erro?.message}`); // Simplificado (ID na coluna)
      ...
      registrador.info(`[Aud] Transcrição recebida da IA. Tamanho: ${resposta?.length || 0}`); // Simplificado (ID na coluna)
      ...
      if (!resultadoEnvio.sucesso) {
           ...
           registrador.error(`[Aud] Falha reportada por servicoMensagem ao enviar resposta: ${resultadoEnvio.erro?.message}`); // Simplificado (ID na coluna)
           // Não lançar erro aqui, pois a IA pode ter funcionado. O problema foi o envio.
      } else {
           registrador.info(`[Aud] Resposta (transcrição) enviada com sucesso.`); // Simplificado (ID na coluna)
      }
      return Resultado.sucesso({ transacao, resposta });

    } catch (erro) {
      registrador.error(`[Aud] ERRO GERAL no processamento: ${erro.message}`, erro); // Simplificado (ID na coluna)

      if (transacaoId) {
          try {
              await gerenciadorTransacoes.registrarFalhaEntrega(transacaoId, `Erro processamento áudio: ${erro.message}`);
          } catch (errTransacao) {
               registrador.error(`[Aud] Falha ao registrar erro na transação: ${errTransacao.message}`); // Simplificado (ID na coluna)
          }
      }
      ...
      if (!msgErroLower.includes('desabilitada') && !msgErroLower.includes('grande') && !msgErroLower.includes('segurança')) {
         try {
            ...
            registrador.error(`[Aud] Falha ao enviar mensagem de erro geral: ${erroEnvio.message}`);
         }
      }
